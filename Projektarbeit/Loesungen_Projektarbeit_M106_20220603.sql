...
TO DISK = N'C:\Program Files\Microsoft SQL Server\MSSQL15.SQLEXPRESS2019\MSSQL\Backup\Backup_per_Script\Northwind.bak' 
...
FROM DISK =  N'C:\Program Files\Microsoft SQL Server\MSSQL15.SQLEXPRESS2019\MSSQL\Backup\Backup_per_Script\Northwind.bak' 
...
CASE
	WHEN YEAR(e.HireDate)= 1993 THEN DATEDIFF(MONTH,HIREDATE, GETDATE())*30 + p.salary + 1000
...
/*Aufgabe 17
Entziehe dem User test sämtliche Berechtigungen. Der User test soll nur auf Objekte im Schema hr und volgende Berechtigungen  SELECT, 
...
SELECT SUM(Quantity*UnitPrice * (1-Discount)) AS GESAMMTUMSATZ  FROM Orders o
JOIN [Order Details] od ON o.OrderID=od.OrderID
WHERE year(o.OrderDate) = 1997


SELECT Sum(od.UnitPrice*Quantity*(1-Discount)) AS ProductSales FROM Products p 
JOIN (Orders o JOIN [Order Details] od ON o.OrderID = od.OrderID) ON p.ProductID = od.ProductID 
WHERE o.ShippedDate BETWEEN '19970101' AND '19971231';

SELECT Sum(od.UnitPrice*Quantity*(1-Discount)) AS ProductSales FROM Products p 
JOIN (Orders o JOIN [Order Details] od ON o.OrderID = od.OrderID) ON p.ProductID = od.ProductID 
WHERE o.ShippedDate >= '19970101' AND o.ShippedDate <='19971231';
